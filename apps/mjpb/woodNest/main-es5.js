...
var AppComponent = /** @class */ (function () {
    ...
    AppComponent.prototype.onFileChange = function (f) {
    ...
    AppComponent.prototype.readFile = function () {
        ...
        reader.onload = function (e) {
            _this.currentFileContent = new DOMParser().parseFromString(e.target.result, 'text/html');
            Array.from(_this.currentFileContent.querySelectorAll('table')).forEach(function (table) {
                var columnIndexes = {};
                Array.from(table.rows).forEach(function (row, index) {
                    var cells = Array.from(row.cells);
                    if (index === 0) {
                        var _loop_1 = function (ctmKey) {
                            var ctm = _this.columnHeaders[ctmKey];
                            columnIndexes[ctm] = cells.indexOf(cells.find(function (cell) { return cell.innerText === ctm; }));
                        };
                        for (var ctmKey in _this.columnHeaders) {
                            _loop_1(ctmKey);
                        }
                    }
                    else {
                        ...
                        var label = {
                            print: true,
                            number: parseInt(cells[columnIndexes[_this.columnHeaders.number]].innerText),
                            subject: cells[columnIndexes[_this.columnHeaders.subject]].innerText,
                            amount: null,
                        };
                        var amount = label.subject.match(/_([0-9]+)stk$/);
    ...
    AppComponent.prototype.getLabelsWithPositions = function () {
        var e_1, _a;
        var labelsWithPos = [];
        try {
            for (var _b = tslib__WEBPACK_IMPORTED_MODULE_0__["__values"](this.labels), _c = _b.next(); !_c.done; _c = _b.next()) {
                var label = _c.value;
                if (label.print === false) {
                    continue;
                }
                for (var i = 1; i <= label.amount; i++) {
                    labelsWithPos.push({
                        label: label,
                        pos: i,
                    });
    ...
    AppComponent.prototype.toggleAll = function () {
        var e_2, _a;
        var toggleTo = this.labels.length ? !this.labels[0].print : false;
        try {
            for (var _b = tslib__WEBPACK_IMPORTED_MODULE_0__["__values"](this.labels), _c = _b.next(); !_c.done; _c = _b.next()) {
                var label = _c.value;
